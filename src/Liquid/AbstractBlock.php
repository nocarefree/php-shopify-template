...
class AbstractBlock extends AbstractTag
{
		...
		while (count($tokens)) {
			...
			if ($startRegexp->match($token)) {
				if ($tagRegexp->match($token)) {
					...
					if ($tagName !== null) {
						$this->nodelist[] = new $tagName($tagRegexp->matches[2], $tokens, $this->fileSystem,  $this->getWhitespaceController($token));
	...
	private function createVariable($token) {
		$variableRegexp = new Regexp('/^' . Liquid::get('VARIABLE_START') . '-?\s(.*)\s-?' . Liquid::get('VARIABLE_END') . '$/');
		if ($variableRegexp->match($token)) {
			return new Variable($variableRegexp->matches[1], $this->getWhitespaceController($token));
		}

		throw new LiquidException("Variable $token was not properly terminated");
	}

	private function getWhitespaceController($token){
		$whitespace = [0,0];
		if(mb_strpos($token,'-') == 2){
			$whitespace[0] = 1;
		}

		if(mb_strrpos($token,'-') == (mb_strlen($token)-3)){
			$whitespace[1] = 1;
		}
		return $whitespace;
	}
